  ...
  <div class="container">
     <div class="searchRecipe-content">
      <form class="form-inline my-2 my-lg-0" @submit.prevent="getApiInfo()">
        <b-form-input class="form-control mr-sm-2" type="search" placeholder="Search Recipes" v-model="input" required></b-form-input>
      </form>
    </div>
    <div class="result">
      <b-card v-for="(result,key) in results" :key='key'
      ...
      <b-card-text>
        ...
        <b-button href="#" variant="primary">Preparation</b-button>
      </b-card>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    getApiInfo (result) {
      ...
      axios.get('https://spoonacular-recipe-food-nutrition-v1.p.rapidapi.com/recipes/search', config)
        .then(response => {
